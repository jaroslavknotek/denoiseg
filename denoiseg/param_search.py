    ...
    for custom in tqdm(configurations_overrides, desc="Running training experiments"):
        ...
        try:
            checkpoint, out_losses = seg.run_training(
                images, ground_truths, train_params, training_output_root, device=device
            )
            ...
            for i, (img, gt, pred, met) in enumerate(
                zip(imgs_test, gts_test, predictions, metrics)
            ):
            ...
            df = pd.DataFrame(list(enumerate(metrics)), columns=["img_id", "metric"])
