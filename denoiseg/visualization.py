...
def sample_ds(images, ground_truths, training_params, weightmaps=None, n=5):
    ...
    dataset = ds.DenoisegDataset(
        images, ground_truths, sample_aug, weightmaps=weightmaps
    )

    for i, t in enumerate(itertools.islice(dataset, 0, n)):
        img = np.squeeze(t["x"])[0]
        mask = np.squeeze(t["y"])[0]
...
def _get_ellipse(features):
    ...
    return matplotlib.patches.Ellipse(
        (x, y), width, height, angle=deg, fill=False, edgecolor=c, lw=2, alpha=0.5
    )
...
def _plot_f1_background(ax, nn=100):
    ...
    f1_grid = (f1_nn.reshape((nn, nn)) % 0.1) > 0.05
    ax.imshow(f1_grid, alpha=0.1, cmap="gray", extent=[0, 1, 1, 0])
