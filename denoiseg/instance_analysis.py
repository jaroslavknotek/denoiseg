...
def sample_precision_recall(ground_truth, foreground, thresholds, component_limit=500):
    thr_imgs = [_threshold_foreground(foreground, thr) for thr in thresholds]

    precs_recs = [
        _calc_prec_rec_from_pred(ground_truth, img, component_limit=component_limit)
        for img in thr_imgs
    ]
...
def evaluate_match(images, ground_truths, predictions, test_names=None):
    ...
    thr_high = 0.8
    n = 5
    thresholds = np.concatenate([[0.0], np.linspace(thr_low, thr_high, n), [1.0]])
...
def extract_instance_properties_df(image, mask):
    ...
    rows = [
        (ins.__dict__ | f.__dict__) for ins, f in zip(segmented_instances, features)
    ]
...
def extract_features(
    ...
    shape_cls = classify_shape(e_height, e_width, circle_radius, pixel_area)
